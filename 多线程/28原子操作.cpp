...
//
//std::atomic<int> g_mycout = 0;//封装了一个类型为原子类型的int对象(值)，我可以像操作普通变量一样
//void mythread()
//{
//	for (int i = 0; i < 1000000; i++)
//	{
//		g_mycout++;//已经是一个atomic对象了，操作(已经是原子操作了)不会被打断
//		
//	}
//}
 ...
 * 原子操作一般指"不可分割的操作"，这个操作要么完成，要么没做，不存在中间状态。
 * c++中使用atomic类模板来实现,其实std::atomic是用来封装某个类型的值的
 * 
 *原子操作--- 基本的用法范例
 *
 *心得：原子操作只针对一个变量(用处有限。。。)，一般用原子操作进行计数，如发送的数据包，
 *接收的数据包
 *
 */
