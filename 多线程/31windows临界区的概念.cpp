...
//本类用于自动释放windows下的临界去，类似于c++的lock_guard()能力。
...
private:
	CRITICAL_SECTION *m_pCritical;//临界区
};
...
public:
	...
	void inMsgRecvQueue()
	{
		for (int i = 0; i < 10000; i++)
		{
			...
			//EnterCriticalSection(&my_winsec);//进入临界区(加锁)，同一个线程允许两次进入 但是退出也得两次(次数匹配)
			//msgRecvQueue.push_back(i);//执行操作
			//LeaveCriticalSection(&my_winsec);//离开临界区(解锁)
			//LeaveCriticalSection(&my_winsec);//离开临界区(解锁),同一个线程允许两次进入 但是退出也得两次(次数匹配)
			//若退出次数少,则说明加锁次数比解锁次数少(会卡住)

			CwinLock wlock(&my_winsec);//调用多次不会出错
			msgRecvQueue.push_back(i);//执行操作
#else			
			//条件编译：这一块的代码不参与编译，也就是说不会在最后的可执行程序里面。
			//linux代码
...
/*
 * 1）windows下的临界区非常类似于c++11中的mutex;临界区使用前必须初始化
 * 需要包含#include<windows.h>
 * (1)进入临界区（和加锁一样）(2)执行操作(3)离开临界区（和解锁一样unlock()）
 * 
 * 2）多次进入临界区实验
 *在同一个线程中，多次进入临界区（相同的临界区变量），就要多次离开临界区，要配对，使用windows临界区是可以的
 不同线程中，第二个进入上述临界区变量(另外一个线程已经进去了 加锁了)准备进入的 会卡住。
 *进入几次临界区就要离开几次临界区。
 
 *c++11中互斥量mutex是不可以加多次锁，解多次锁。--相同的mutex不允许在同一个线程中调用多次。否则会崩溃
 *
 *
 *3）自动析构技术
