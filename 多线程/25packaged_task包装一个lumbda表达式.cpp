...
int main(void)
{
	...
	////////////////////////////////////////////////////////////////////////////
	myTasks.push_back(std::move(mpt));//移动语义，执行完后mpt会变空。
	//复制
	std::packaged_task<int(int)>mypt2;
	auto iter = myTasks.begin();
	mypt2 = std::move(*iter);//移动语义
	myTasks.erase(iter);//删除第一个元素，迭代器已经失效，不能在继续使用
	mypt2(123);//直接调用，相当于函数调用(不会创建新的线程。)
	//创建线程的写法
	//thread t2(std::ref(myTask), 20);//千万不能少了这个myTask，否则会编译报错error。
	//t2.join();
...
*是个类模板，它的模板参数是各种可调用对象，通过packageed_task可以把各种可调用对象包装起来，方便作为线程入口函数来调用。
